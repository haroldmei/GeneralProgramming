...
void readUsers()
{
	CString strName = CString("HKEY_LOCAL_MACHINE\\SAM\\SAM\\Domains\\Account\\Users\\Names");
	CString strDefault = CString("t400");
	//RegisterOps::access(Action pAction, string pKey, char * pField, FiledType pType, ValueList &pResult);
	CString strResult;
	CRegistry reg;

	strResult = reg.ReadString(strName, strDefault);
	return;
}
...
BOOL CRegistry::SetCurrentPath(CString strKey)
{
...
BOOL CRegistry::RegDelnode (HKEY hKeyRoot, LPTSTR lpSubKey)
{
...
BOOL CRegistry::DeleteKey(CString strKey)
{
	...
	if (!strKey.Find("HKEY_CURRENT_USER", 0))
	{
		TRACE("1\n");
		hRootKey = HKEY_CURRENT_USER;
	}
	else if (!strKey.Find("HKEY_CLASSES_ROOT", 0))
	{
		TRACE("2\n");
		hRootKey = HKEY_CLASSES_ROOT;
	}
	else if (!strKey.Find("HKEY_LOCAL_MACHINE", 0))
	{
		TRACE("3\n");
		hRootKey = HKEY_LOCAL_MACHINE;
	}
	else if (!strKey.Find("HKEY_USERS", 0))
	{
		TRACE("4\n");
		hRootKey = HKEY_USERS;
	}
	...
	int   nBeg   =   strKey.Find( '\\');
	int   nEnd   =   strKey.GetLength();
	CString strKeySub   =   strKey.Mid(nBeg+1,   nEnd   -   nBeg);
	return RegDelnode(hRootKey, LPTSTR(strKeySub.GetBuffer()));
	//return TRUE;
}
...
BOOL CRegistry::DeleteValue(CString strName)
{
	...
	do{
		nBeg   =   strKeySub.Find( '\\');
		nEnd   =   strKeySub.GetLength();
		strKeySub   =   strKeySub.Mid(nBeg+1,   nEnd   -   nBeg);
	}while(nBeg != -1);
	strValueSub = strKeySub;
	strKeySub = strName.Mid(0,   strName.GetLength()   -   strKeySub.GetLength());
	


	//HKEY hRootKey = HKEY_CURRENT_USER;
	if (!strName.Find("HKEY_CURRENT_USER", 0))
	{
		TRACE("1\n");
		m_hRootKey = HKEY_CURRENT_USER;
	}
	else if (!strName.Find("HKEY_CLASSES_ROOT", 0))
	{
		TRACE("2\n");
		m_hRootKey = HKEY_CLASSES_ROOT;
	}
	else if (!strName.Find("HKEY_LOCAL_MACHINE", 0))
	{
		TRACE("3\n");
		m_hRootKey = HKEY_LOCAL_MACHINE;
	}
	else if (!strName.Find("HKEY_USERS", 0))
	{
		TRACE("4\n");
		m_hRootKey = HKEY_USERS;
	}
	else
	{
	}
	...
	nBeg   =   strKeySub.Find( '\\');
	nEnd   =   strKeySub.GetLength();
	CString strCurPath   =   strKeySub.Mid(nBeg+1,   nEnd   -   nBeg);
	SetCurrentPath(strCurPath);
	...
	lResult = ::RegDeleteValue(hKey, LPCTSTR(strValueSub));
	::RegCloseKey(hKey);
