...
HTREEITEM CViewUserOutput::AddItem(HTREEITEM hParent, LPCTSTR szText, int iImage, int enType)
{
    TVITEM tvi = {0};
...
{  
    ...
    GetItem(tvItem);
    int enPRType = (int)GetItemData(tvItem.hItem);
...
void CViewUserOutput::OnInitialUpdate()
{
    ...
    DeleteAllItems();
    HTREEITEM htiRoot = AddItem(NULL, _T("人员列表"), 0, 1);
    AddItem(htiRoot, _T("被检人员"), 0, 1);
    /*AddItem(htiRoot, _T("PRLIST_SUSPECTED_NAME"), 0, 1);
    AddItem(htiRoot, _T("PRLIST_NORMAL_NAME"), 0, 1);
    AddItem(htiRoot, _T("PRLIST_EXCEPTION_NAME"), 0, 1);
