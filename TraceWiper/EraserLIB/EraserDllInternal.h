...
{
    eraserTraceBase("eraserAddError\n");
    CString strError = CString("UNKNOWN ERROR");;
    try {
...
#define eraserBeginNotifyNoAccess(x) \
    if ((x)->m_hwndWindow != NULL) { \
        PostMessage((x)->m_hwndWindow, (x)->m_uWindowMessage, ERASER_WIPE_BEGIN, (LPARAM)x); \
    }
...
#define eraserUpdateNotifyNoAccess(x) \
    if ((x)->m_hwndWindow != NULL) { \
        PostMessage((x)->m_hwndWindow, (x)->m_uWindowMessage, ERASER_WIPE_UPDATE, (LPARAM)x); \
    }
...
#define eraserTestPausedNotifyNoAccess(x) \
    if ((x)->m_hwndWindow != NULL) { \
        PostMessage((x)->m_hwndWindow, (x)->m_uWindowMessage, ERASER_TEST_PAUSED, (LPARAM)x); \
    }
...
{
    E_UINT32 uBlockSize = max(context->m_piCurrent.m_uCluster,
...
#endif // #ifndef ERASER_INTERNAL_H
