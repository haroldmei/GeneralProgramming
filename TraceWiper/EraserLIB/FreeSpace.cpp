...
wipeFreeSpace(CEraserContext *context)
{
    ...
    try {
        ...
        eraserRemoveFolderA((LPVOID)(LPCTSTR)strFolder, (E_UINT16)strFolder.GetLength(),
        ...
        if (CreateDirectory((LPCTSTR)strFolder, NULL)) {
            ...
            try {
                ...
                bResult = eraserOK(eraserGetFreeDiskSpaceA((LPVOID)context->m_piCurrent.m_szDrive,
                ...
                if (bResult) {
                    ...
                    for (E_UINT32 uCurrent = 1; uCurrent <= uFiles; uCurrent++) {
                        ...
                        if (context->m_hFile != INVALID_HANDLE_VALUE) {
                            ...
                            try {
                                if (uCurrent == uFiles) {
                                    if (eraserOK(eraserGetFreeDiskSpaceA((LPVOID)context->m_piCurrent.m_szDrive,
                                            (E_UINT16)lstrlen(context->m_piCurrent.m_szDrive),
                                            &context->m_uiFileSize.QuadPart))) {
            ...
            if (eraserError(eraserRemoveFolderA((LPVOID)(LPCTSTR)strFolder,
            ...
            /*eraserAddError1(context, IDS_ERROR_DIRECTORY,
                            (LPCTSTR)context->m_piCurrent.m_szDrive);	waba*/
            context->m_saFailed.Add(context->m_strData);
        }
    } catch (CException *e) {
        handleException(e, context);
