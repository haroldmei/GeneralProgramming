...
getInfo_FAT32(wfeInfo *pwInfo, E_PUINT8 pBuffer)
{
	...
	pwInfo->m_uSectorsPerFAT     = (E_UINT16)((bs32->bpb.A_BF_BPB_BigSectorsPerFatHi << 16) + bs32->bpb.A_BF_BPB_BigSectorsPerFat);
	...
	if (pwInfo->m_uSectorsTotal == 0) {
		pwInfo->m_uSectorsTotal  = (bs32->bpb.A_BF_BPB_BigTotalSectorsHigh << 16) + bs32->bpb.A_BF_BPB_BigTotalSectors;
	}
	pwInfo->m_uFATDirectoryStart = (bs32->bpb.A_BF_BPB_RootDirStrtClusHi << 16) + bs32->bpb.A_BF_BPB_RootDirStrtClus;
	pwInfo->m_uStartSector       = pwInfo->m_uReservedSectors + pwInfo->m_uSectorsPerFAT * bs32->bpb.A_BF_BPB_NumberOfFATs;
}
...
getInfo_FAT(wfeInfo *pwInfo, E_PUINT8 pBuffer)
{
	...
	pwInfo->m_uFATDirectorySize  = (E_UINT16)((32 * bs->bsRootDirEnts + pwInfo->m_uSectorSize - 1) / pwInfo->m_uSectorSize);
	pwInfo->m_uFATDirectoryStart = pwInfo->m_uReservedSectors + pwInfo->m_uSectorsPerFAT * bs->bsFATs;
...
initFileSystemInfo(wfeInfo *pwInfo, PARTITIONINFO& pi)
{
	...
	if (pwInfo->m_uSectorsPerCluster == 0) {
		...
		pwInfo->m_uClustersTotal = (pwInfo->m_uSectorsTotal - pwInfo->m_uStartSector) / pwInfo->m_uSectorsPerCluster;
	}
