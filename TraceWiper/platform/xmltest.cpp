...
int main()
{
        XmlTree *lTree = XmlTree::getInstance();
/*
		TiXmlDocument doc( "f:/tinyxml/demo.xml" );
		bool loadOkay = doc.LoadFile();

		if ( !loadOkay )
		{
			printf( "Could not load test file 'demo.xml'. Error='%s'. Exiting.\n", doc.ErrorDesc() );
			exit( 1 );
		}
*/
#if 0
...
#endif
#if 0
        TiXmlElement *RootElement = lTree->rootElement();
		TiXmlNode* node = 0;
		TiXmlElement* todoElement = 0;
//		TiXmlElement* itemElement = 0;
        ...
        int count = 0;
        std::cout<<"root : "<<RootElement->Value()<<std::endl;
		for(node =  RootElement->FirstChild(); node; node = node->NextSibling())
		{
            todoElement = node->ToElement();
            printElement(todoElement);
            printChild(todoElement);
			count++;
		}
#endif
#if 0
        ...
        lTree->treeNodes(&lTreeNames);

        for(unsigned int i = 0; i < lTreeNames.size(); i++)
        {
            NameList lSubName;
            cout<<lTreeNames[i]<<" : "<<endl;
            lTree->subTreeNodes(lTreeNames[i], &lSubName);
            for(unsigned int k = 0; k < lSubName.size(); k++)
            {
                NameList lName;
                cout<<"    "<<lSubName[k]<<" :"<<endl;
                lTree->nodes(lTreeNames[i],  lSubName[k], &lName);
                for(unsigned int j = 0; j < lName.size(); j++)
                {
                    cout<<"        name:  "<<lName[j]<<endl;
                }                
            }
        }

#endif
    extern bool excute(std::string& pNodeName, int pCmd);
string lName = "腾讯QQ";
//    excute(lName, 1);
extern  void indexFileView(CString pFile);
indexFileView("c:/Documents and Settings/qinchaox/Local Settings/History/History.IE5/index.dat");
//indexFileView("c:/Documents and Settings/qinchaox/Local Settings/Temporary Internet Files/Content.IE5/index.dat");


    //test();
//extern void printGuiTreeNode(s_TREENODE & pNode);

//    s_TREENODE lNodeTree;
//    XmlTree::getInstance()->guiTree(lNodeTree);
//    printGuiTreeNode(lNodeTree);
    return 0;
}

void printGuiTreeNode(s_TREENODE & pNode)
{
    int i = 0;
    cout<<" NodeId : "<<pNode.dwNodeID<<endl;
    cout<<" Name   : "<<pNode.sNodeName.GetBuffer(0)<<endl;
    for(i = 0; i < pNode.vChildNodes.size(); i++)
    {
        printGuiTreeNode(pNode.vChildNodes[i]);
    }
}
...
{ 
    ...
    if (cValues) 
    {
        LPBYTE   pbbinKeyData =   new   BYTE[cbMaxValueData];   

        printf( "\nNumber of values: %d\n", cValues);

        for (i=0, retCode=ERROR_SUCCESS; i<cValues; i++) 
        { 
            DWORD lDataLen = cbMaxValueData;
            DWORD lpType = 0;
...
void xmain(void)
{
   ...
   if( RegOpenKeyEx( HKEY_CURRENT_USER,
        TEXT("SOFTWARE\\Microsoft\\CTF\\MSUTB"),
        0,
        KEY_READ,
...
bool excute(std::string& pNodeName, int pCmd)
{
    Titles lTitles;
...
//    resultEmpty();
    ...
    for(unsigned int i = 0; i < lTitles.size(); i++)
    {
        ...
        {
            switch(lTitle.mKeys[keyNo]->mAccess)
            {
                case Register:
                {
                    for(unsigned int j = 0; j < lTitle.mKeys[keyNo]->mFields.size(); j++)
                    {
                        Field &lField = lTitle.mKeys[keyNo]->mFields[j];
                        ValueList lResult;
                        RegisterOps::getInstance()->access(lField.mAction, lTitle.mKeys[keyNo]->mKey, (char*)lField.mField.c_str(), lField.mType, lResult);
                        //keep result
                        if(lResult.size() == 0)
                        {
    ...
    for( unsigned int field = 0; field < fields; field++)
    {
        ...
        {
            std::string lTmpStr = titleKeys[j][field];
            cout<<lTmpStr<<" "<<endl;
            lList.push_back(titleKeys[j][field]);
        }
        cout<<"add row"<<endl;
//        resultAddRow(&lList);
    }
